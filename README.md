...
Provide every variable in `.env.example`. `GEMINI_API_KEY`, signing keys, and database credentials are server-only and must never use a `NEXT_PUBLIC_` prefix.
...
- `POST /api/jobs/:jobId/retry` — re-fund and retry a previously refunded provider job without losing ledger history.
...
Credits are not granted merely because a transaction is mined. The chain adapter waits until Monad's `finalized` block reaches the settlement block, then rechecks the consumed session hash before running the atomic ledger transition.
...
The Monad creative is an independent hackathon sample campaign, not an official paid Monad advertisement. Each user can receive at most one successful reward from a campaign. `campaign_reward_claims` separates that durable reward invariant from restartable quest attempts: the original verified quest can retry a failed settlement, while a different task cannot farm the same campaign.
